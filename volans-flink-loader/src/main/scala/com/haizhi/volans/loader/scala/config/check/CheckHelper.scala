...
object CheckHelper {
  ...
  def checkHive(map: util.Map[String, AnyRef]): Unit = {
    ...
    if (!map.containsKey(SinksParameter.DEL_Flag))
      throw new VolansCheckException(s"${ErrorCode.PARAMETER_CHECK_ERROR}${ErrorCode.PATH_BREAK}   HIVE sink  [${SinksParameter.DEL_Flag}] Key field missing")
