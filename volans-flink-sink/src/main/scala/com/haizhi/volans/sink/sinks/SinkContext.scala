...
import com.flink.sink.{ArangoDBSink, HbaseSink}
import com.google.gson.reflect.TypeToken
import com.haizhi.volans.common.flink.base.scala.util.JSONUtils
import com.haizhi.volans.sink.config._
import com.haizhi.volans.sink.constant.{CoreConstants, StoreType}
import com.haizhi.volans.sink.schema.SchemaVo
...
object SinkContext {
  ...
  def parseArgs(sinkCfg: String): Unit = {
    val jsonMap = JSONUtils.jsonToMap(sinkCfg)
  ...
  def getSinkStoreTypes(): Seq[StoreType] = {
    ...
    sinks.map(sink => sink.storeType).toSeq
  }
   ...
   * @return
   */
  private def buildSinks(sinkCfg: AnyRef): List[Sink] = {
    val sinks: mutable.Set[Sink] = new mutable.HashSet[Sink]()
    val sinkList: util.List[util.Map[String, AnyRef]] = JSONUtils.fromJson(JSONUtils.toJson(sinkCfg), new TypeToken[util.List[util.Map[String, AnyRef]]]() {}.getType)
